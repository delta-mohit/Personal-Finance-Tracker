...
export async function scanReceipt(file) {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash", apiKey: process.env.GEMINI_API_KEY });
    ...
    const jsonString = response.candidates[0].content.parts[0].text.replace(/```json\n|```/g, '').trim();
